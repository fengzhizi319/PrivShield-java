...
#
# 用法 / Usage: ./console/scripts/dev-start.sh [--force]
...
launch_agent() {
    ...
    echo "启动 PrivShield Java Agent (REST: $AGENT_URL)，日志: $agent_log..."
    local jar_path
    ...
    (
        ...
        exec java -jar "$jar_path" \
            --server.port=8079 >> "$agent_log" 2>&1
    ) &
    AGENT_PID=$!
    PIDS[0]="$AGENT_PID"
...
echo ""
echo "======================================================================"
echo "🚀【开发模式】 Python REST 代理控制台已成功启动！"
echo "======================================================================"
echo "  前端 UI (Vite HMR):  $VITE_URL  <-- 请访问此地址获得毫秒级热更新！"
echo "  Python 代理后端 (API): $CONSOLE_URL"
