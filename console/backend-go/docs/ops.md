...
如果你想先从“前端 / 后端 / agent / 服务器整体怎么配”的角度理解开发模式与商业化产品模式，请先看 `../../docs/modes.md`；本文这一节聚焦 Go 后端自身该怎么切换。
...
在商业化产品模式下，Go 后端通常更偏向二进制交付、mTLS 保护上游 agent 链路、通过反向代理或 Ingress 对外暴露，并关闭开发期的临时参数与调试日志。
